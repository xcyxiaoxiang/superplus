...
## Exploration Reports（可选输出）

探索过程中，如果你发现需要形成一份可归档的探索报告，可以参考 `skills/exploring/templates/exploration-report-reference.md`。这份模板提供了结构化的参考格式，但**非强制**。

**何时使用参考模板**：
- 探索内容较多，希望形成一份可回溯的探索报告时
- 需要向他人（或未来的自己）清晰呈现探索过程时
- 项目复杂度较高，需要结构化整理发现时
...
参考模板包含 10 个建议 section：元数据、技术基础、现有系统分析、外部接口规范、字段映射分析、完整业务链路、配置项清单、开发模块清单、待确认事项、使用指南。根据项目复杂度裁剪使用。

**记住**：探索的本质是自由思考，不是填写表单。如果模板阻碍了思考，就丢掉它。
